...
$Y_m$, with a $\nabla X_m$ driving force, leads in a nonlinear, coupled Crank-Nicolson update, the
...
\begin{equation}
  ...
  = A_h^{(k+1)} + D_{T,AD}^{(k+1)} + H_{AD}^{(k+1)} + \half \Big( D_T^n - D_m^{(k)} + H^n - H^{(k)} \Big)
 \label{eq:hup}
\end{equation}
where
\begin{eqnarray*}
...
However, rather than computing $h_{{\rm AD}}^{(k+1)}$ directly, we solve this iteratively, based on the approximation
...
\end{eqnarray}
where $H_{AD}^{(k+1,\ell)} = - \nabla \cdot \sum h_m(T_{AD}^{(k+1),\ell})$. Note that again the solve for this
