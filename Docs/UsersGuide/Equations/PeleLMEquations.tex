...
\begin{equation}
  \frac{\rho^{n+1,(k+1)} - \rho^n}{\Delta t} = A_{\rho}^{(k)} = \sum A_{m}^{(k)}
...
We next compute $\Gamma_m^{(k)}$, which are conservatively corrected versions of 
...
Once all the species equations are updated with (\ref{eq:pY}), compute $\Gamma_{m,{\rm AD}}^{(k+1)}$, which are
conservatively corrected versions of $\widetilde\Gamma_{m,{\rm AD}}^{(k+1)}$,
and then compute the updated species mass fractions, $Y_{m,{\rm AD}}^{(k+1)}$, using
\begin{eqnarray}
  \frac{\rho^{(k+1)}Y_{m,{\rm AD}}^{(k+1)} - (\rho Y_m)^n}{\Delta t}
  &=& A_m^{{k}} + D_{m,AD}^{(k+1)} + \half(D_m^n - D_m^{(k)}) + I_{R,m}^{(k)}
\end{eqnarray}
where
\begin{eqnarray*}
   &&D_{m,AD}^{(k+1)} = - \nabla \cdot \Gamma_{m,{\rm AD}}^{(k+1)}
\end{eqnarray*}
...
\begin{equation}
  ...
  = A_h^{(k)} + D_{T,AD}^{(k+1)} + H^{(k+1)} + \half \Big( D_T^n - D_m^{(k)} + H^n - H^{(k)} \Big)
 \label{eq:hup}
\end{equation}
where
\begin{eqnarray*}
  &D^n = - \nabla \cdot \lambda^n \nabla T^n   \hspace{2cm}
  &H^n = - \nabla \cdot \sum h_m(T^n) \; \Gamma_m^n\\
  &D^{(k)} = - \nabla \cdot \lambda^{(k)} \nabla T^{{k}}
  &H^{(k+1)} = - \nabla \cdot \sum h_m(T^{(k+1)}) \; \Gamma_m^{(k+1)}\\
  &D_{T,AD}^{(k+1)} = \nabla \cdot \lambda_{AD}^{(k+1)} \nabla T_{AD}^{(k+1)}
  &H^{(k)} = - \nabla \cdot  \sum h_m(T^{(k)}) \; \Gamma_m^{(k)}
\end{eqnarray*}
...
\rho^{(k+1)} C_p^{(k+1),\ell} \delta T^{\ell +1}  &-& \frac{\Delta t}{2} \nabla \cdot \lambda^{(k+1),\ell} \nabla \delta T^{\ell +1} \nonumber  \\
 &=& \rho^n h^n - \rho^{(k+1)} h^{(k+1),\ell} + \Delta t \Big( A_h^{(k)} + D_{T,AD}^{(k+1)} + H^{(k+1)} \Big) \\
