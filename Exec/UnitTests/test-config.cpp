...
TEST(Configuration, Build)
{
  ...
  amrex::Print() << "PeleLM SHA = " << pc_git << std::endl
...
#else
  amrex::Print() << "PeleLM not built with MPI support." << std::endl;
  GTEST_SKIP();
#endif
}
...
TEST(Configuration, CUDA)
{
  ...
  std::cout << "[" << myrank << "] " << rankDevice << ": " << dev.name
            ...
            << std::endl;
#else
  amrex::Print() << "PeleLM not built with CUDA support" << std::endl;
  GTEST_SKIP();
#endif
}
