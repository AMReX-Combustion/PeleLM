...
for l=1:nlev
  ...
  for n=1:num_grids
    ...
    if (xlo == xloc_min)
      ...
      Nusselt_left_box = (xloc_max/((Th-Tc)))*(Th-temperature_left);
      Nusselt_left(n) = trapz(ylo+dy/2:dy:yhi-dy/2,Nusselt_left_box');
    endif
    
    if (xhi == xloc_max)
      temp_dat=dat{1}{n};
      temperature_right=temp_dat(:,32);
      Nusselt_right_box = (xloc_max/((Th-Tc)))*(Tc-temperature_right);
