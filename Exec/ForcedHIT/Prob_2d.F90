...
!#define BL_PIPEEND  7
    ...
    if ( (probtype(1:len).eq.BL_PROB_PREMIXED_FIXED_INFLOW) &
     ...
     else if (probtype(1:len).eq.BL_PROB_FORCEDHIT) then
      ...
      if ((probtype(1:len).eq.BL_PROB_FORCEDHIT)) then
         ...
         do j = lo(2), hi(2)
            ...
            do i = lo(1), hi(1)
               ...
               do n=1,Nspec
                  scal(i,j,FirstSpec-1+n) = Y_bc(n-1,airZone)
               enddo
               scal(i,j,Temp) = T_bc(airZone)
...
! ::: -----------------------------------------------------------
!
!     This routine adds the forcing terms to the momentum equation
!
...
! !     Let's do a forced HIT case, where we add A*u_i to the momentum equation
!       Aforce = 10.d0

!       do j = jlo, jhi
!          do i = ilo, ihi
!             force(i,j,nXvel) = Aforce*rho(i,j)
