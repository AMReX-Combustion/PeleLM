...
! to not override the calc_divu already declared in PeleLM.cpp
    ...
    REAL_T Y(nspecies), H(nspecies), cpmix, rho, rhoInv, mmw, invmwt(nspecies)
...
!   Comute lambda.Grad(T)
    ...
    do j=lo(2),hi(2)
      do i=lo(1),hi(1)+1
         Fx(i,j,nspecies+3) = - rhoDx(i,j)*(T(i,j) - T(i-1,j))* dxInv * Ax(i,j)
      enddo
    enddo

    do j=lo(2),hi(2)+1
      do i=lo(1),hi(1)
         Fy(i,j,nspecies+3) = - rhoDy(i,j)*(T(i,j) - T(i,j-1)) * dyInv * Ay(i,j)
      enddo
    enddo
...
!   Compute hi*Fi
    ...
    do n=1,nspecies
       do j=lo(2),hi(2)
       do i=lo(1),hi(1)+1
          Fx(i,j,nspecies+2) = Fx(i,j,nspecies+2) + Fx(i,j,n)*(H(i,j,n)+H(i-1,j,n))*0.5d0
       enddo
       enddo
    enddo
    ...
    do n=1,nspecies
       do j=lo(2),hi(2)+1
       do i=lo(1),hi(1)
          Fy(i,j,nspecies+2) = Fy(i,j,nspecies+2) + Fy(i,j,n)*(H(i,j,n)+H(i,j-1,n))*0.5d0
       enddo
       enddo
    enddo
...
!   xlo
    if (lo(1).eq.dlo(1) .and. Tbc(1,1).eq.EXT_DIR) then
       i = dlo(1)
       Fx(i,lo(2):hi(2),nspecies+2) = 0.d0
       do n=1,nspecies
          do j=lo(2),hi(2)
             Fx(i,j,nspecies+2) = Fx(i,j,nspecies+2) + Fx(i,j,n)*H(i-1,j,n)
          enddo
      enddo
    endif
!   xhi
    if (hi(1).eq.dhi(1) .and. Tbc(1,2).eq.EXT_DIR) then
       ...
       do n=1,nspecies
          do j=lo(2),hi(2)
             Fx(i,j,nspecies+2) = Fx(i,j,nspecies+2) + Fx(i,j,n)*H(i,j,n)
          enddo
       enddo
    endif
!   ylo
    if (lo(2).eq.dlo(2) .and. Tbc(2,1).eq.EXT_DIR) then
       j = dlo(2)
       Fy(lo(1):hi(1),j,nspecies+2) = 0.d0
       do n=1,nspecies
          do i=lo(1),hi(1)
             Fy(i,j,nspecies+2) = Fy(i,j,nspecies+2) + Fy(i,j,n)*H(i,j-1,n)
          enddo
       enddo
    endif
...
!   yhi
    if (hi(2).eq.dhi(2) .and. Tbc(2,2).eq.EXT_DIR) then
       ...
       do n=1,nspecies
          do i=lo(1),hi(1)
             Fy(i,j,nspecies+2) = Fy(i,j,nspecies+2) + Fy(i,j,n)*H(i,j,n)
          enddo
       enddo
    endif
  ...
  integer function pphys_TfromHY(lo, hi, T, DIMS(T), &
      ...
      do j=lo(2),hi(2)
         do i=lo(1),hi(1)
            ...
            !write(*,*) 'DEBUG in pphys_TfromHY', i,j,T(i,j),HMIX(i,j),Yt
