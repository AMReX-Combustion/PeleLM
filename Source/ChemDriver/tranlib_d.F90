...
!C
      ...
      PARAMETER (CFMT='(8A16)', IFMT='(10I12)', LFMT='(L8)', RFMT='(1P,5E24.16)')
!C
...
!C
      ...
      ' TRANLIB:  CHEMKIN-III MULTICOMPONENT TRANSPORT LIBRARY,', &
      PREC(1:CKLSCH(PREC)), ' PRECISION Vers. ', &
      PRVERS(1:CKLSCH(PRVERS)+1), PRDATE, &
...
!C
      IF (KERR .OR. (.NOT.POK) .OR. (.NOT.VOK)) THEN
         IF (KERR) THEN
            WRITE (LOUT,'(/A,/A)') &
           ' There is an error in the transport linkfile...', &
           ' Check TRANFIT output for error conditions.'
         ENDIF
         IF (.NOT. VOK) THEN
            WRITE (LOUT,'(/A,A)') &
           ' Transport linkfile is incompatible with Transport', &
           ' Library Version 3.6'
         ENDIF
         IF (.NOT. POK) THEN
            WRITE (LOUT, '(/A,A)') &
           ' Precision of Transport linkfile does not agree with', &
...
!C
  999 CONTINUE
      WRITE (LOUT, 50)
   50 FORMAT (' Error reading Transport linkfile.')
      REWIND (LINKMC)
      IFLAG = NREC
      RETURN
 1111 CONTINUE
...
!C     end of SUBROUTINE MCSAVE
  ...
  999 CONTINUE
!C
      WRITE (LOUT, *) ' Error writing Transport linkfile information...'
      RETURN
      END
!C                                                                      C
!C----------------------------------------------------------------------C
